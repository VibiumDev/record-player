...
function isSensitiveField(selector) {
  ...
  return /passw|secret|token|api.?key|auth|credential|ssn|social.?sec|credit.?card|card.?num|cvv|cvc|pin\b|otp\b/.test(
    s,
  );
}
...
function humanizeAction(action, mask = true) {
  ...
  if (n.includes("navigate") || n.includes("goto")) {
    try {
      return "Navigate to " + new URL(p.url).hostname;
    } catch {
      return "Navigate to " + (p.url || "page");
    }
  }
...
function parseUrlParams() {
  try {
    ...
    if (atRaw) {
      ...
      if (mMatch) {
        atMs =
          (parseInt(mMatch[1]) * 60 + parseInt(mMatch[2])) * 1000 +
          (mMatch[3] ? parseInt(mMatch[3].padEnd(2, "0")) * 10 : 0);
    ...
    return {
      timeline:
        timeline === "visible" || timeline === "v" ? true : timeline === "hidden" || timeline === "h" ? false : null,
      inspector:
        inspector === "visible" || inspector === "v"
          ? true
          : inspector === "hidden" || inspector === "h"
            ? false
            : null,
      controls:
        controls === "visible" || controls === "v" ? true : controls === "hidden" || controls === "h" ? false : null,
      at: atMs,
    };
  } catch {
    return { timeline: null, inspector: null, controls: null, at: null };
  }
}
...
function normalizeActionCoords({ action, screenshot, viewport, dpr, imgW, imgH, natW, natH }) {
  ...
  if (snapshotViewport?.width && snapshotViewport?.height) {
    ...
    if (scrollX || scrollY) {
      addCandidate(
        candidates,
        snapshotViewport.width,
        snapshotViewport.height,
        scrollX,
        scrollY,
  ...
  if (!snapshotViewport && screenshot?.width && screenshot?.height) {
    ...
    if (scrollX || scrollY) {
      addCandidate(
        ...
        scrollX,
        scrollY,
        "screencast-frame-viewport-scroll-sub",
      );
      addCandidate(
        candidates,
        screenshot.width,
        ...
        -scrollX,
        -scrollY,
        "screencast-frame-viewport-scroll-add",
      );
    }
  ...
  const inferredBoosts = Array.from(
    new Set(
      [
        ...
        ...(targetViewport?.height && screenshot?.height
          ? [Math.round(screenshot.height / targetViewport.height)]
          : []),
  ...
  for (const c of candidates) {
    ...
    if (box) {
      ...
      if (!pointInBox) {
        ...
        if ((scrollX || scrollY) && !c.ratioMode) {
          ...
          const pointInAdjBox =
            normPx >= adjBx - 2 && normPx <= adjBx + adjBw + 2 && normPy >= adjBy - 2 && normPy <= adjBy + adjBh + 2;
    ...
    if (penalty < bestPenalty) {
      ...
      best = {
        ...
        source: c.source,
        boost: c.boost,
        coordW: c.coordW / c.boost,
        coordH: c.coordH / c.boost,
        bx,
        by,
        bw,
        bh,
...
const ActionOverlay = forwardRef(function ActionOverlay(
  { action, screenshot, viewport, dpr, imgEl, containerEl, showDebug, layoutKey },
  _ref,
) {
  const [, forceUpdate] = useState(0);
  useEffect(() => {
    const t = setTimeout(() => forceUpdate((n) => n + 1), 250);
    return () => clearTimeout(t);
  }, [layoutKey]);
...
const RecordStudio = forwardRef(function RecordStudio(_props, _ref) {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      mqH.removeEventListener("change", onH);
    };
  }, []);
  ...
  useEffect(() => {
    if (mobile || compact) {
      ...
      setShowDetail(false);
      if (layoutMode !== "stacked") setShowSide(false);
    }
  ...
  const levelColors = useMemo(
    () => ({ log: V.textMid, info: brand.amber, warn: brand.sunset, error: "#f87171", warning: brand.sunset }),
    [V],
  );
  ...
  useEffect(() => {
    ...
    const onMove = (e) => {
      ...
      if (
        (sideDrag.current || timelineDrag.current || detailDrag.current || stackedDrag.current || dragging.current) &&
        e.cancelable
      ) {
        e.preventDefault();
      }
  ...
  useEffect(() => {
    try {
      localStorage.setItem("record-panel-inspector", showSide);
    } catch {}
  }, [showSide]);
  useEffect(() => {
    try {
      localStorage.setItem("record-panel-timeline", showTimeline);
    } catch {}
  }, [showTimeline]);
  useEffect(() => {
    try {
    ...
    const onKey = (e) => {
      ...
      if (key === "ArrowLeft" || key === "ArrowUp") {
        ...
        for (let i = navStops.length - 1; i >= 0; i--) {
          if (navStops[i].time < playhead - 10) {
            prev = navStops[i];
            break;
          }
        }
      ...
      if (key >= "0" && key <= "9") {
        e.preventDefault();
        setPlayhead((traceData.duration * parseInt(key)) / 10);
        setIsPlaying(false);
      }
      if (key === "h" || key === "H") {
        e.preventDefault();
  ...
  if (!traceData) {
    return (
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div
            ...
            style={{
              ...
              boxShadow: logoSpinning ? `0 0 0 3px ${V.orange}44, 0 0 16px 4px ${V.orange}66` : "none",
            }}
          >
            <img
              src={VIBIUM_LOGO_HI}
        ...
        <div
          style={{ color: V.textDim, fontSize: 14, maxWidth: 440, textAlign: "center", lineHeight: 1.6, marginTop: 8 }}
        >
          Use{" "}
          <code style={{ background: V.bgCard, padding: "1px 4px", borderRadius: 3, color: V.amber }}>Vibium API</code>,{" "}
          <code style={{ background: V.bgCard, padding: "1px 4px", borderRadius: 3, color: V.amber }}>MCP</code>, or{" "}
          <code style={{ background: V.bgCard, padding: "1px 4px", borderRadius: 3, color: V.amber }}>CLI</code> +{" "}
          <code style={{ background: V.bgCard, padding: "1px 4px", borderRadius: 3, color: V.orange }}>Skill</code> to
          generate recordings.
          <br />
          <a
            ...
            style={{ color: V.orange, textDecoration: "none" }}
          >
          ...
          >
            ...
            >
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
                ...
                <span
                  onClick={() => setShowHelp(false)}
                  style={{ cursor: "pointer", color: V.textDim, fontSize: 20, lineHeight: 1 }}
                >
                  ✕
                </span>
              </div>
    ...
    >
        ...
        >
          ...
          >
            ...
            {mobile && (
              <button
                onClick={() => {
                  setIsPlaying(false);
                  const prev = [...filteredActions].reverse().find((a) => a.startTime < playhead - 10);
            ...
            {mobile && (
              <button
                onClick={() => {
                  setIsPlaying(false);
                  const next = filteredActions.find((a) => a.startTime > playhead + 10);
                  if (next) {
                    setPlayhead(next.endTime || next.startTime);
          ...
          {!mobile && (
            ...
            >
              <input
                ...
                style={{ width: 52, accentColor: V.orange, height: 2, cursor: "pointer", outline: "none" }}
              />
              <span
                style={{
                  fontSize: 12,
          ...
          {!mobile && (
            <button
              ...
              style={{
                background: layoutMode === "stacked" ? V.orange + "18" : "none",
                border: layoutMode === "stacked" ? `1px solid ${V.orange}40` : "1px solid transparent",
                color: layoutMode === "stacked" ? V.orange : V.textDim,
                cursor: "pointer",
                padding: "3px 8px",
                borderRadius: 6,
                fontSize: 20,
      ...
      >
        ...
        >
              ...
              if (Math.abs(dx) > 40 && Math.abs(dx) > Math.abs(dy) * 1.5) {
                ...
                if (dx < 0) {
                  ...
                  for (let i = sorted.length - 1; i >= 0; i--) {
                    if (sorted[i].startTime < playhead - 10) {
                      prev = sorted[i];
                      break;
                    }
                  }
          ...
          >
            ...
            {overlayEnabled && playhead > 0 && currentGroup && (
              ...
              >
                <span style={{ fontSize: 14, fontWeight: 700, color: V.purple, letterSpacing: "0.02em" }}>
                  {currentGroup.title}
                </span>
              </div>
            )}
            {currentScreenshot?.url ? (
              <>
                <img
                  ...
                  onLoad={(e) => {
                    setImgDims({ w: e.currentTarget.clientWidth, h: e.currentTarget.clientHeight });
                  }}
                  style={{
                    maxWidth: "100%",
                    maxHeight: "100%",
                  ...
                  <>
                    <div
                      ...
                      style={{
                        ...
                        cursor: "pointer",
                        margin: "0 auto 16px",
                        boxShadow: `0 0 30px ${V.orange}40`,
                        transition: "transform 0.15s, box-shadow 0.15s",
                      }}
                      onMouseEnter={(e) => {
                        e.currentTarget.style.transform = "scale(1.08)";
                        e.currentTarget.style.boxShadow = `0 0 40px ${V.orange}60`;
                      }}
                      onMouseLeave={(e) => {
                        e.currentTarget.style.transform = "scale(1)";
                        e.currentTarget.style.boxShadow = `0 0 30px ${V.orange}40`;
                      }}
                    >
                    ...
                    <div style={{ fontSize: 13, color: V.textDim }}>
                      {traceData.actions.length} actions · {(traceData.duration / 1000).toFixed(1)}s ·{" "}
                      {traceData.screenshots.filter((s) => s.url).length} frames
                    </div>
                    <div style={{ fontSize: 12, color: V.textDim, marginTop: 8 }}>
                      {mobile
                        ? "Swipe ← → to step through actions"
                        : "Press play or use ← → to step through actions · L/D to toggle theme"}
                    </div>
                  </>
                )}
              </div>
            ...
            {overlayEnabled && playhead > 0 && currentAction && (
              <div
                style={{
                  ...
                  border: `1px solid ${actionColor(currentAction.apiName)}50`,
                  borderRadius: 8,
                  padding: "8px 16px",
                  display: "flex",
                  alignItems: "center",
              ...
              >
                <span
                  style={{
                    fontSize: 14,
                    fontWeight: 700,
                    color: actionColor(currentAction.apiName),
                    whiteSpace: "nowrap",
                  }}
                >
                  {humanizeAction(currentAction)}
                </span>
              </div>
            )}
          </div>
        ...
        {layoutMode === "stacked" && showSide && (
          <div
            onMouseDown={(e) => {
              if (e.target.dataset.chevron) return;
              e.preventDefault();
              stackedDrag.current = true;
              stackedDragStart.current = {
                y: e.clientY,
                h: typeof screenshotH === "number" ? screenshotH : (mainAreaRef.current?.clientHeight || 400) * 0.5,
              };
        ...
        {showSide ? (
          <>
            ...
            <div
              style={{
                width: layoutMode === "stacked" ? undefined : sideW,
                flex: layoutMode === "stacked" ? 1 : undefined,
                flexShrink: 0,
                background: V.bgPanel,
                display: "flex",
                flexDirection: "column",
                ...
                {[
                  ...
                  <button
                    ...
                    style={{
                      ...
                      color: activePanel === tab.key ? tab.color : V.textDim,
                      cursor: "pointer",
                      padding: "10px 0",
                      fontSize: 13,
                      fontWeight: 600,
                ...
                >
                  ...
                  {[
                    ...
                    <button
                      ...
                      style={{
                        flex: 1,
                        background: actionFilter === f.key ? V.orange + "18" : "transparent",
                        border: `1px solid ${actionFilter === f.key ? V.orange + "40" : V.border}`,
                        color: actionFilter === f.key ? V.orange : V.textDim,
                        cursor: "pointer",
                        padding: "4px 0",
                        borderRadius: 4,
                        fontSize: 13,
                  ...
                  (() => {
                    ...
                    return allItems.map((item, i) => {
                      if (item.type === "group-start") {
                        ...
                        const childCount = filteredActions.filter(
                          (a) => a.startTime >= g.startTime && a.endTime <= g.endTime,
                        ).length;
                        return (
                          <div
                            key={`gs-${i}`}
                            ...
                            style={{
                              ...
                              marginBottom: 2,
                              cursor: "pointer",
                              borderLeft: `3px solid ${V.purple}${isActive ? "" : "50"}`,
                          ...
                          >
                            ...
                            <span
                              style={{ fontSize: 13, fontWeight: 700, color: isActive ? V.purple : V.textMid, flex: 1 }}
                            >
                              {g.title}
                            </span>
                            {isCollapsed && (
                            ...
                            {isCollapsed &&
                              playhead >= g.startTime &&
                              playhead <= g.endTime &&
                              (() => {
                                const groupDuration = g.endTime - g.startTime;
                                const progress =
                                  groupDuration > 0
                                    ? Math.min(1, Math.max(0, (playhead - g.startTime) / groupDuration))
                                    : 0;
                                return (
                                  <div
                                    style={{
                      ...
                      return (
                        <div
                          key={`a-${i}`}
                          ref={
                            isActive || selectedAction === a
                              ? (el) => el?.scrollIntoView?.({ block: "center", behavior: "smooth" })
                              : undefined
                          }
                          onClick={() => {
                            setPlayhead(a.endTime || a.startTime);
                            setSelectedAction(a);
                          }}
                          style={{
                            position: "relative",
                        ...
                        >
                          ...
                          {(isActive || selectedAction === a) &&
                            playhead >= a.startTime &&
                            (() => {
                              const end = a.endTime || a.startTime;
                              const duration = end - a.startTime;
                              const progress =
                                duration > 0
                                  ? Math.min(1, Math.max(0, (playhead - a.startTime) / duration))
                                  : playhead >= a.startTime
                                    ? 1
                                    : 0;
                              return (
                  ...
                  traceData.network.map((n, i) => {
                    ...
                    return (
                      <div
                        ...
                        onClick={() => setPlayhead(n.startTime)}
                        style={{
                          display: "flex",
                          alignItems: "center",
                          gap: 5,
                      ...
                      >
                        <span
                          style={{
                            fontSize: 12,
                            fontWeight: 700,
                            color: isActive ? c : statusColor(n.status),
                            minWidth: 22,
                          }}
                        >
                          {n.status || "—"}
                        </span>
                        <span style={{ fontSize: 12, fontWeight: 600, color: V.textDim, minWidth: 26 }}>
                          {n.method}
                        </span>
                        <div
                          style={{
                            flex: 1,
                            fontSize: 13,
                            color: isActive ? V.text : V.textMid,
                            overflow: "hidden",
                            textOverflow: "ellipsis",
                            whiteSpace: "nowrap",
                          }}
                  ...
                  <div style={{ padding: 6, fontSize: 13 }}>
                              ...
                              {metaRows.map(([label, value], i) => (
                                <div
                                  ...
                                  style={{
                                    ...
                                    gap: 8,
                                    padding: "3px 0",
                                    borderBottom: `1px solid ${V.border}30`,
                                  }}
                                >
                                  <span style={{ fontSize: 12, color: V.textDim, minWidth: 70, flexShrink: 0 }}>
                                    {label}
                                  </span>
                                  <span style={{ fontSize: 12, color: V.textMid, wordBreak: "break-all" }}>
                                    {value}
                                  </span>
                                </div>
                              ))}
                    ...
                    <div style={{ marginBottom: 12 }}>
                      ...
                      {[
                        ...
                        <div
                          key={i}
                          style={{ display: "flex", gap: 8, padding: "3px 0", borderBottom: `1px solid ${V.border}30` }}
                        >
                ...
                <>
                  ...
                  {showDetail && (
                    ...
                    >
                      ...
                      >
                        <div style={{ fontSize: 14, fontWeight: 600, color: actionColor(selectedAction.apiName) }}>
                          {selectedAction.apiName}
                        </div>
                        <span
                          onClick={() => setMaskSensitive((m) => !m)}
                          style={{
                            fontSize: 11,
                            color: maskSensitive ? V.textDim : V.orange,
                            cursor: "pointer",
                            padding: "1px 6px",
                            borderRadius: 4,
                            border: `1px solid ${maskSensitive ? V.border : V.orange}40`,
                          }}
                        >
                      ...
                      {selectedAction.error && (
                        <div
                          style={{
                            fontSize: 13,
                            color: "#ef4444",
                            background: dark ? "#450a0a80" : "#fef2f240",
                            padding: "4px 6px",
                            borderRadius: 4,
                            marginBottom: 4,
                          }}
                        >
                          {typeof selectedAction.error === "string"
                            ? selectedAction.error
                            : selectedAction.error?.message || JSON.stringify(selectedAction.error)}
                        </div>
                      )}
                      <pre
                        style={{
                  ...
                  return (
                    <div
                        ...
                        active
                          ? (el) => {
                              if (el && isPlaying && filmstripRef.current) {
                                ...
                                const left =
                                  el.offsetLeft - container.offsetLeft - container.clientWidth / 2 + el.offsetWidth / 2;
                      ...
                      style={{
                        ...
                        cursor: "pointer",
                        boxShadow: active ? `0 0 16px ${V.orange}60` : "none",
                      }}
                    >
          ...
          <div style={{ height: timelineH, flexShrink: 0, background: V.bg, overflow: "hidden" }}>
            <div
              ...
              onTouchStart={(e) => {
                dragging.current = true;
                const t = e.touches[0];
                handleScrub({ clientX: t.clientX, clientY: t.clientY, currentTarget: e.currentTarget });
              }}
            >
              <div style={{ width: `${100 * zoom}%`, minWidth: "100%", position: "relative" }}>
                ...
                {(() => {
                  ...
                  const labelStyle = {
                    ...
                    alignItems: "center",
                    paddingLeft: 6,
                    fontSize: Math.max(7, Math.min(10, Math.round(8 * s))),
                    fontWeight: 600,
                    pointerEvents: "none",
                    background: V.bg,
                    borderRight: `1px solid ${V.border}20`,
                  };
                      ...
                      <div style={{ height: rulerH, position: "relative", borderBottom: `1px solid ${V.border}` }}>
                        <div
                          style={{
                            ...labelStyle,
                            color: V.textDim,
                            fontSize: Math.max(6, Math.min(9, Math.round(7 * s))),
                          }}
                        >
                          Time
                        </div>
                            ...
                            return (
                              <div
                                ...
                                style={{
                                  ...
                                  width: `max(${w}%, 16px)`,
                                  height: actionBarH,
                                  background: a.error ? "#ef444430" : isSelected ? `${c}40` : `${c}25`,
                                  border: `1px solid ${a.error ? "#ef4444" : isSelected ? c : `${c}40`}`,
                                  borderRadius: Math.round(3 * Math.min(s, 1.5)),
                                  display: "flex",
                                  alignItems: "center",
                                  padding: "0 3px",
                                  overflow: "hidden",
                            ...
                            return (
                              <div
                                ...
                                style={{
                                  ...
                                  top: "50%",
                                  transform: "translate(-50%, -50%)",
                                  width: c.type === "error" ? dotErrSize : dotSize,
                                  height: c.type === "error" ? dotErrSize : dotSize,
                                  borderRadius: c.type === "error" ? 2 : "50%",
                                  background: color,
      ...
      {!compact && !mobile && showTimeline && (
        ...
        >
          {!footerNarrow && (
            <>
              <span>
                <span style={{ color: traceData.actions.some((a) => a.error) ? "#ef4444" : "#22c55e" }}>●</span>{" "}
                {traceData.actions.some((a) => a.error) ? "Has errors" : "OK"}
              </span>
              <span>
                {filteredActions.length}
                {actionFilter !== "all" ? `/${traceData.actions.length}` : ""} actions
              </span>
              ...
              <div style={{ flex: 1 }} />
              <span>
                {traceData.actions.filter((a) => a.error).length} action error
                {traceData.actions.filter((a) => a.error).length === 1 ? "" : "s"}
              </span>
              <span>
                {traceData.network.filter((n) => n.status >= 400).length} HTTP error
                {traceData.network.filter((n) => n.status >= 400).length === 1 ? "" : "s"}
              </span>
              <span>
                {traceData.console.filter((c) => c.type === "error").length} console error
                {traceData.console.filter((c) => c.type === "error").length === 1 ? "" : "s"}
              </span>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
            ...
            <input
              ...
              style={{ width: 60, accentColor: V.orange, height: 2, cursor: "pointer", outline: "none" }}
            />
          </div>
        </div>
      )}
